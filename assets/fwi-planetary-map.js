/* FutureWorld Intelligence Planetary Computer map integration v1.2 */
(function(){
  'use strict';
  ...
  const LEGENDS={
    ndmi:{
      title:'Moisture condition (NDMI)',unit:'Index from -1 to +1',
      rows:[
      ...
      howTo:'Brown tones indicate a weaker moisture signal in vegetation or exposed surfaces. Blue-green tones indicate a stronger moisture signal.',
      warning:'NDMI is a surface and vegetation moisture indicator. It is not, by itself, a declaration of meteorological, agricultural or household water drought.'
    },
    ndvi:{
      title:'Vegetation condition (NDVI)',unit:'Index from -1 to +1',
      rows:[
      ...
      howTo:'Red and orange tones indicate little vegetation response. Green tones indicate a stronger vegetation response for the selected period.',
      warning:'Low NDVI may reflect season, bare land, settlements, harvested crops, fire, cloud contamination or drought. Field and time-series checks are required.'
    },
    context:{
      title:'Satellite context',unit:'Natural-colour imagery',rows:[],
      howTo:'The map shows the landscape as seen by the selected satellite rendering. Use roads, settlements, vegetation and surface-water features for orientation.',
      warning:'Natural-colour imagery does not directly measure drought, emissions, vulnerability or policy implementation.'
    }
  };
  ...
  const INDICATORS={
    'water-drought':[
      {
        key:'ndmi',label:'Moisture condition — NDMI',collection:'sentinel-2-l2a',legend:'ndmi',lockCollection:true,
        evidenceClass:'Operational satellite indicator',
        explanation:'NDMI compares near-infrared and short-wave infrared reflectance to show relative moisture conditions in vegetation and exposed surfaces.',
        render:{name:'Normalized Difference Moisture Index (NDMI)',description:'Sentinel-2 B08 and B11 moisture index',options:'assets=B08&assets=B11&expression=(B08-B11)/(B08+B11)&asset_as_band=true&rescale=-0.8,0.8&colormap_name=BrBG'},
        requiredEvidence:'Confirm drought with rainfall, climatic water balance, soil-moisture, streamflow, groundwater and local water-access evidence.'
      },
      {
        key:'ndvi',label:'Vegetation condition — NDVI',collection:'sentinel-2-l2a',legend:'ndvi',lockCollection:true,
        evidenceClass:'Supporting satellite indicator',
        explanation:'NDVI compares near-infrared and red reflectance to show vegetation response and possible stress patterns.',
        render:{name:'Normalized Difference Vegetation Index (NDVI)',description:'Sentinel-2 B08 and B04 vegetation index',options:'assets=B08&assets=B04&expression=(B08-B04)/(B08+B04)&asset_as_band=true&rescale=-0.2,0.9&colormap_name=RdYlGn'},
        requiredEvidence:'Compare the same season across years and combine with rainfall, land use, fire and field observations.'
      },
    ...
    default:[
      {key:'context',label:'Satellite context',collection:'sentinel-2-l2a',legend:'context',lockCollection:false,evidenceClass:'Visual context only',explanation:'The current public map supplies satellite context while a validated thematic layer is prepared.',renderMatcher:/natural color|true color/i,requiredEvidence:'This theme requires additional authoritative indicator, risk or policy datasets.'}
    ]
  };
    ...
    root.innerHTML=`
      <div class="fwi-pc-map-shell">
        <div class="fwi-pc-map-toolbar" aria-label="Planetary Computer map controls">
          ...
          <label class="fwi-pc-indicator-control"><span>Indicator</span><select data-pc-control="indicator">
            ${indicatorList(themeKey).map(item=>`<option value="${item.key}">${escapeHtml(item.label)}</option>`).join('')}
          </select></label>
          <label><span>Geography</span><select data-pc-control="geography">${Object.entries(PRESETS).map(([key,preset])=>`<option value="${key}">${preset.label}</option>`).join('')}</select></label>
          ...
          <div class="fwi-pc-map-actions"><button type="button" class="neon-btn" data-pc-action="load">Load evidence for current view</button><button type="button" class="ghost-btn" data-pc-action="clear">Clear evidence</button></div>
        </div>
        <div class="fwi-pc-evidence-summary" data-pc-summary>
          <div><small>Selected indicator</small><strong>${escapeHtml(firstIndicator.label)}</strong><span>Load the layer to inspect Bajaur or the selected map view.</span></div>
          <div><small>What it measures</small><strong>${escapeHtml(firstIndicator.evidenceClass)}</strong><span>${escapeHtml(firstIndicator.explanation)}</span></div>
          <div><small>How to use it</small><strong>Interpret with the legend</strong><span>Click the map after loading to request a location-specific value.</span></div>
        ...
        <div class="fwi-pc-map-stage">
          ...
          <aside class="fwi-pc-map-legend" data-pc-legend aria-live="polite"></aside>
          <div class="fwi-pc-map-status" data-pc-status aria-live="polite"><span class="ready"></span><strong>Map ready.</strong> Select a theme, indicator and focused area, then load evidence.</div>
        </div>
        <div class="fwi-pc-map-meta" data-pc-meta>
          <div><small>Evidence theme</small><strong>${escapeHtml(THEMES[themeKey].label)}</strong></div>
          <div><small>Indicator</small><strong>${escapeHtml(firstIndicator.label)}</strong></div>
          ...
          <div><small>Verification</small><strong>Field and time-series review required</strong></div>
        </div>
        <div class="fwi-pc-map-notice"><strong>Use responsibly:</strong> the moisture and vegetation layers are satellite-derived indicators. They do not by themselves prove meteorological drought, water scarcity for households, restoration success, hazard severity or causation. Compare like seasons and add rainfall, hydrological, land-use and field evidence.</div>
      </div>`;
  }
  ...
  function initMap(root){
    ...
    function updateThemeGuidance(){
      ...
      controls.guidance.innerHTML=`<div><small>${escapeHtml(indicator.evidenceClass)}</small><strong>${escapeHtml(indicator.label)}</strong><span>${escapeHtml(indicator.explanation)} ${escapeHtml(theme.note)}</span></div><a href="${THEME_BASE}#${encodeURIComponent(controls.theme.value)}">Open full theme guide</a>`;
    }

    function updateSummary(sample=null){
      const indicator=selectedIndicator();
      let sampleTitle='No location sampled';
      let sampleText='Load the evidence layer, then click the map to request a location-specific value.';
      if(sample){sampleTitle=sample.title;sampleText=sample.text}
      controls.summary.innerHTML=`
        <div><small>Selected indicator</small><strong>${escapeHtml(indicator.label)}</strong><span>${escapeHtml(indicator.explanation)}</span></div>
        <div><small>Selected location</small><strong>${escapeHtml(sampleTitle)}</strong><span>${escapeHtml(sampleText)}</span></div>
        <div><small>How to read it</small><strong>Use the map legend</strong><span>${escapeHtml(selectedLegend().howTo)}</span></div>
        <div><small>Additional evidence needed</small><strong>Validation required</strong><span>${escapeHtml(indicator.requiredEvidence)}</span></div>`;
    }

    function renderLegend(loaded=false){
      const legend=selectedLegend();
      controls.legend.innerHTML=`<button type="button" class="fwi-pc-legend-toggle" data-pc-legend-toggle aria-expanded="true">Theme guide & legend</button><div class="fwi-pc-legend-body"><small>${escapeHtml(selectedTheme().label)}</small><h3>${escapeHtml(legend.title)}</h3><p class="fwi-pc-legend-state">${loaded?'Evidence layer loaded':'Select “Load evidence” to display this layer.'}</p>${legend.rows.length?`<div class="fwi-pc-legend-scale">${legend.rows.map(row=>`<div><i style="--legend-color:${row.color}"></i><span><strong>${escapeHtml(row.label)}</strong><small>${escapeHtml(row.range)}</small></span></div>`).join('')}</div>`:''}<p>${escapeHtml(legend.howTo)}</p><p class="fwi-pc-legend-warning"><strong>Important:</strong> ${escapeHtml(legend.warning)}</p></div>`;
    ...
    function drawExtent(preset){if(extentLayer)map.removeLayer(extentLayer);const [west,south,east,north]=preset.bbox;extentLayer=L.rectangle([[south,west],[north,east]],{color:'#f2b544',weight:1.5,opacity:.9,fillOpacity:.035,dashArray:'7 6',interactive:false}).addTo(map)}
    function applyPreset(key){const preset=PRESETS[key]||PRESETS.bajaur;const [west,south,east,north]=preset.bbox;map.fitBounds([[south,west],[north,east]],{padding:[18,18],animate:true});drawExtent(preset);setStatus('ready','View updated.',`${preset.label} is a search extent; zoom or pan before loading if needed.`)}
    function currentBbox(){const bounds=map.getBounds();return [bounds.getWest(),bounds.getSouth(),bounds.getEast(),bounds.getNorth()].map(value=>Number(value.toFixed(6)))}
    function validateSearch(bbox){const [west,south,east,north]=bbox;if(!controls.start.value||!controls.end.value)throw new Error('Select both start and end dates.');if(controls.start.value>controls.end.value)throw new Error('The start date must be before the end date.');if((east-west)>MAX_LONGITUDE_SPAN||(north-south)>MAX_LATITUDE_SPAN||map.getZoom()<5)throw new Error('The current view is too large for high-resolution evidence. Choose Khyber Pakhtunkhwa, Merged Districts or Bajaur, or zoom in further.')}

    function updateMeta(data){
      const cards=[['Evidence theme',selectedTheme().label],['Indicator',selectedIndicator().label],['Evidence class',selectedIndicator().evidenceClass],['Dataset',data.dataset],['Display mode',data.mode],['Scenes matched',String(data.sceneCount)],['Observation range',data.observationRange],['Rendering',data.rendering],['Upstream provider',data.providers],['Collection licence',data.license],['Verification','Satellite indication; climate and field review required']];
    ...
    function removeImagery(){if(imageryLayer){map.removeLayer(imageryLayer);imageryLayer=null}activePointBase='';activePointParams=null;renderLegend(false)}
    function addTileLayer(tileJson,label){if(!tileJson||!Array.isArray(tileJson.tiles)||!tileJson.tiles[0])throw new Error('The tile service returned no usable map tiles.');removeImagery();imageryLayer=L.tileLayer(tileJson.tiles[0],{minZoom:Number.isFinite(tileJson.minzoom)?tileJson.minzoom:0,maxZoom:Number.isFinite(tileJson.maxzoom)?tileJson.maxzoom:24,opacity:Number(controls.opacity.value)/100,attribution:`${escapeHtml(label)} via <a href="https://planetarycomputer.microsoft.com/" target="_blank" rel="noopener">Microsoft Planetary Computer</a>`}).addTo(map);imageryLayer.bringToFront();if(extentLayer)extentLayer.bringToFront();renderLegend(true)}

    function selectBestItem(features){return [...features].sort((a,b)=>{const cloudA=Number(a.properties?.['eo:cloud_cover']??999);const cloudB=Number(b.properties?.['eo:cloud_cover']??999);if(cloudA!==cloudB)return cloudA-cloudB;return new Date(b.properties?.datetime||0)-new Date(a.properties?.datetime||0)})[0]}
    ...
    function setIndicatorOptions(){
      ...
      controls.indicator.value=items[0].key;
      applyIndicator(false);
    }

    async function applyIndicator(reload=true){
      const indicator=selectedIndicator();
      controls.collection.value=indicator.collection||'sentinel-2-l2a';
      controls.collection.disabled=Boolean(indicator.lockCollection);
      removeImagery();lastMeta=null;updateThemeGuidance();updateSummary();renderLegend(false);
      await loadRenderOptions();
      if(reload)setStatus('ready',`${indicator.label} selected.`,indicator.requiredEvidence);
    }
    ...
    async function loadRenderOptions(){
      ...
      renderOptions=[];
      if(indicator.render){
        renderOptions=[indicator.render];
      }else{
      ...
      controls.render.disabled=renderOptions.length===1;
    ...
    async function buildSingleItem(features,renderConfig){
      ...
      if(!selected?.id)throw new Error('No suitable scene was available for fallback rendering.');
    ...
    function numericValue(payload){
      const candidates=[];
      if(Array.isArray(payload?.values))candidates.push(...payload.values.flat(Infinity));
      if(Array.isArray(payload?.assets))payload.assets.forEach(asset=>{if(Array.isArray(asset?.values))candidates.push(...asset.values.flat(Infinity))});
      for(const value of candidates){const number=Number(value);if(Number.isFinite(number))return number}
      return null;
    }

    function interpretValue(value){
      const key=selectedIndicator().key;
      if(key==='ndmi'){
        if(value<-.2)return 'Very low moisture signal';
        if(value<.1)return 'Low moisture signal';
        if(value<.25)return 'Transitional or mixed moisture signal';
        if(value<.4)return 'Moderate moisture signal';
        return 'High moisture signal';
      }
      if(key==='ndvi'){
        if(value<.1)return 'Very sparse or non-vegetated signal';
        if(value<.25)return 'Sparse vegetation signal';
        if(value<.45)return 'Moderate vegetation signal';
        if(value<.65)return 'Healthy vegetation signal';
        return 'Very strong vegetation signal';
      }
      return 'Context image; no classified index value';
    }

    async function samplePoint(latlng,openPopup=true){
      const indicator=selectedIndicator();
      if(!activePointBase||!activePointParams||indicator.key==='context'){
        const text=indicator.key==='context'?'Natural-colour context has no single drought value. Select NDMI or NDVI for an index.':'Load an operational indicator before sampling a location.';
        if(openPopup)L.popup().setLatLng(latlng).setContent(`<strong>${escapeHtml(indicator.label)}</strong><br>${escapeHtml(text)}`).openOn(map);
        updateSummary({title:'No numeric value',text});
        return;
      }
      try{
        const params=new URLSearchParams(activePointParams.toString());
        const pointUrl=activePointBase.includes('/mosaic/')?`${activePointBase}/point/${latlng.lng.toFixed(6)},${latlng.lat.toFixed(6)}?${params.toString()}`:`${activePointBase}/point/${latlng.lng.toFixed(6)},${latlng.lat.toFixed(6)}?${params.toString()}`;
        const payload=await requestJson(pointUrl);
        const value=numericValue(payload);
        if(value===null)throw new Error('No valid pixel value was returned for this location.');
        const category=interpretValue(value);
        updateSummary({title:`${category} (${value.toFixed(3)})`,text:`Location ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}. ${selectedIndicator().requiredEvidence}`});
        if(openPopup)L.popup({maxWidth:360}).setLatLng(latlng).setContent(`<strong>${escapeHtml(indicator.label)}</strong><br><b>Value:</b> ${value.toFixed(3)}<br><b>Interpretation:</b> ${escapeHtml(category)}<br><small>${escapeHtml(selectedLegend().warning)}</small>`).openOn(map);
      }catch(error){
        const text='The map layer loaded, but the public point-sampling service did not return a usable value. Read the colour legend and try another location.';
        updateSummary({title:'Point value unavailable',text});
        if(openPopup)L.popup().setLatLng(latlng).setContent(`<strong>${escapeHtml(indicator.label)}</strong><br>${escapeHtml(text)}`).openOn(map);
      }
    }
    ...
    async function loadImagery(){
      ...
      controls.load.disabled=true;controls.load.textContent='Loading thematic evidence…';
      setStatus('loading',`Loading ${selectedIndicator().label}…`,'Planetary Computer processing may take several seconds.');
      ...
      try{
        ...
        activeRender=selectedRender;
        let mode='Planetary Computer mosaic';let result;let selectedItem=null;
        ...
        activePointBase=result.pointBase;activePointParams=result.pointParams;
        addTileLayer(result.tileJson,datasetName);
        ...
        setStatus('success','Evidence layer loaded.',`${selectedIndicator().label}; ${mode}; ${sceneCount} matching scene${sceneCount===1?'':'s'}.${selectedNote} Click the map for a location value.`);
        await samplePoint(map.getCenter(),false);
      }catch(error){
        console.error('FWI Planetary Computer thematic map error',error);
        setStatus('error','Unable to load thematic evidence.',error.name==='AbortError'?'The request timed out. Please try a smaller area.':error.message);
      }finally{controls.load.disabled=false;controls.load.textContent='Load evidence for current view'}
    }

    controls.theme.addEventListener('change',()=>{setIndicatorOptions();setStatus('ready',`${selectedTheme().label} selected.`,selectedTheme().note)});
    ...
    controls.collection.addEventListener('change',()=>{removeImagery();lastMeta=null;loadRenderOptions();setStatus('ready','Dataset changed.','Load evidence to refresh the map.')});
    controls.render.addEventListener('change',()=>{activeRender=renderOptions[Number(controls.render.value)]||renderOptions[0];removeImagery();lastMeta=null;setStatus('ready','Rendering changed.','Load evidence to refresh the map.')});
    ...
    controls.clear.addEventListener('click',()=>{removeImagery();lastMeta=null;updateSummary();setStatus('ready','Evidence cleared.','The base map remains active.')});
    map.on('click',event=>samplePoint(event.latlng,true));
    map.on('moveend',()=>{const selected=PRESETS[controls.geography.value];if(selected){const center=map.getCenter();if(Math.abs(center.lat-selected.center[0])>.7||Math.abs(center.lng-selected.center[1])>.7)controls.geography.value=''}});

    updateThemeGuidance();updateSummary();renderLegend(false);applyPreset(presetKey);applyIndicator(false);setTimeout(()=>map.invalidateSize(),250);
  }

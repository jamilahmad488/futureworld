...
**Rollout authorized:** 15 July 2026  
...
- Citation title, author, language and public URL metadata. Include a machine-readable publication year/date only when it is supported by the record; otherwise show “Not recorded” publicly and use `n.d.` in the recommended citation.
...
1. Use the approved Force No. 1 pilot as the reference implementation.
2. Preserve the inventory classification and transparently label unresolved numbering rather than inventing a final number.
3. Use the shared publication-identity styles and controlled page manifest.
4. Apply the standard category by category following the controlled inventory.
5. Validate identity, canonical, citation, social and structured metadata for every page.
6. Validate each publication separately; do not bulk-assign validated status.
7. Keep all retrospectively classified pages at “Retrospective validation pending” until their evidence, citation, originality, disclosure and human-approval records are complete.
